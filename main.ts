...
console.log(chalk.bold.magenta.italic("\n \t Welcome to Code-With-Areeba-OOP-My-Bank\n"));
...
async function bankService(bank:Bank){
    ...
    let res = await inquirer.prompt({
        ...
        if(account){
            ...
            console.log(`Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic(name?.lastName)} Your Account Balance is ${chalk.bold.blueBright
                (`$${account.balance}`
     ...
     if (service.Select == "Cash Withdraw"){
        ...
        if(account){
            ...
            // transaction method
            bank.transaction({accNumber:account.accNumber,balance:
            newBalance });
